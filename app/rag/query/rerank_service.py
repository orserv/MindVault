...
def get_data_and_validates(state):
    ...
    if not rewritten_query or len(rrf_chunks) == 0 or len(web_search_docs) == 0:
        logger.error(
            f"rewritten_query,rrf_chunks,web_search_docs可能为空,业务无法继续进行,提前终止!")
        raise ValueError(
            f"rewritten_query,rrf_chunks,web_search_docs可能为空,业务无法继续进行,提前终止!")
...
def create_question_answer_list(rewritten_query, reranker_docs) -> list[list[str]]:
    ...
    rewritten_query_tokens_list = tokenizer.encode(
        rewritten_query, add_special_tokens=False)
    ...
    for doc in reranker_docs:
        ...
        answer_token_len = len(tokenizer.encode(
        ...
        if rewritten_query_token_len + answer_token_len + 4 > RERANK_MAX_INPUT_TOKENS:
            ...
            limit = max(RERANK_MIN_SUMMARY_CHARS, int(
                (RERANK_MAX_INPUT_TOKENS - 4 - rewritten_query_token_len) / RERANK_SUMMARY_CHAR_RATIO))
            # 加载提示词
            rerank_text_refine_str = load_prompt(
                "rerank_text_refine", question=rewritten_query, answer=answer, limit=limit)
            # 封装message
            messages = [
                HumanMessage(
...
def use_reranker_deal_score(question_answer_pair_list, reranker_docs):
    ...
    scores_list = reranker_model.compute_score(
        question_answer_pair_list, normalize=True)
...
def rerank_documents(state: QueryGraphState) -> QueryGraphState:
    ...
    """
    # 1.获取并且校验参数(state) rewritten_query  rrf_chunks  web_search_docs
    rewritten_query, rrf_chunks, web_search_docs = get_data_and_validates(
        state)
    # 2. 数据格式化处理
    reranker_docs = deal_rrf_and_web_result(rrf_chunks, web_search_docs)
    # 3. 组装问题和答案的列表(rewritten_query,reranker_list) -> question_answer_pair_list [[],[],[]]
    question_answer_pair_list: list[list[str]] = create_question_answer_list(
        rewritten_query, reranker_docs)
    # 4. 打分+排序
