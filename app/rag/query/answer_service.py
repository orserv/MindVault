...
def state_exists_answer(state) -> bool:
    ...
    if is_stream:
        # 是流式 现在就要结果给推送队列中
        push_to_session(state.get("session_id"),
                        SSEEvent.DELTA, {"delta": answer})

    logger.info(f"answer内容不为空,前期没有识别出item_name,提前回答!")
    return True
...
def load_answer_prompt(state: QueryGraphState) -> str:
    ...
    # history
    history_text = ""
    message_list = history_repository.list_recent(
        state.get("session_id"), limit=6)
    # 正确支撑 -> item_names明确
    if not message_list or len(message_list) == 0:
        logger.warning(
            f"当前会话:{state.get('session_id')}没有历史对话记录!提前跳出,history_text为空!")
        history_text = "无对话记录!"
    else:
        final_message_list = [
            item
        ...
        if not final_message_list or len(final_message_list) == 0:
            logger.warning(
                f"当前会话:{state.get('session_id')}没有有效的历史对话记录!提前跳出,history_text为空!")
...
def call_llm_deal_answer(state, answer_prompt_text):
    ...
    if is_stream:
        ...
        for chunk in stream:
            # 增量数据 delta -> 队列中
            push_to_session(state.get("session_id"),
                            SSEEvent.DELTA, {"delta": chunk.content})
...
def extract_text_image_url(state):
    ...
    for doc in reranked_docs:
        ...
        if url and url.endswith(tuple(SUPPORTED_IMAGE_EXTENSIONS)):
...
def generate_answer(state: QueryGraphState) -> QueryGraphState:
    ...
    # base_answer = state.get(
    #     "answer") or f"这是关于「{state.get('original_query', '当前问题')}」的测试回答，正在演示打字机流式输出效果。噜啦噜啦啦啦啦啦啦！！！"
    # final_text = ""
    # image_urls = []
    # if is_stream:
