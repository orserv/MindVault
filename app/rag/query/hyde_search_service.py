...
def search_by_milvus(item_names: list[str], rewritten_query: str, answer: str):
    ...
    result = llm_provider.generate_embeddings([rewritten_query+":"+answer])
...
def call_llm_answer(rewritten_query) -> str:
    ...
    # 2. 加载提示词字符串
    hyde_prompt_text = load_prompt(
        "hyde_prompt", rewritten_query=rewritten_query)
    # 3. 封装提示词Message
    messages = [
        HumanMessage(
...
def search_by_hyde(state: QueryGraphState) -> QueryGraphState:
    ...
    # 3. 进行向量的混合+条件检索(item_names rewritten_query) -> [{id/chunk_id/pk:x,distance:0.9,entity:{输出的field} }]
    milvus_list = search_by_milvus(item_names, rewritten_query, llm_answer)
    # 4. 进行结果的统一格式化处理( [{id/chunk_id/pk:x,distance:0.9,entity:{输出的field} }]) -> [{id:x,输出的field,score:,type:milvus,url:""},{},{}]
