...
from app.process.import_.agent.main_graph import import_app, ImportGraphState
...
app = FastAPI(
    title=infra_config.settings_config.import_app_name,
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=list(infra_config.settings_config.cors_origins) or ["*"],
    allow_methods=["*"],
    allow_headers=["*"],
)
...
def import_html():
    ...
    """
    html_path = PROJECT_ROOT / "app" / "resources" / "page" / "import.html"
    print(f"[INFO] 返回导入演示页面：{html_path.name}")
    return FileResponse(path=html_path, media_type=guess_type(html_path.name)[0])


def invoke_import_graph(task_id:str, local_file_path:str, local_file_dir:str):
    # 执行必须要state -> 必须要三个参数  task_id / local_file_path  / local_file_dir
    try:
        # processing
        update_task_status(task_id,status_name=TASK_STATUS_PROCESSING)
        state:ImportGraphState = create_default_state(
            task_id=task_id,local_file_path=local_file_path,local_file_dir=local_file_dir
         )
...
async def uploads(
    ...
    files: list[UploadFile] = File(...)  # 使用 File 显式声明
):
    """
    本质: 异步调用图对象,解析本次上传的文件
         backgroundtasks.add_task(函数,参数)
 
    1. 先定一个一个执行graph的函数 (task_id,local_file_path,local_file_dir)
    2. 凑齐参数 task_id,local_file_path,local_file_dir
    ...
    :return:
    """
    # 1. 凑齐参数 task_id,local_file_path,local_file_dir
    ...
    # local_file_dir ->  项目根路径 / output / 20260718 / task_id
    time_now_str =  datetime.now().strftime("%Y%m%d")
    local_file_dir_obj:Path = PROJECT_ROOT / "output" / time_now_str / task_id
    local_file_dir_obj.mkdir(parents=True,exist_ok=True)
    # files[0] -> file (hk180烫金机使用手册.pdf) -> 运行内存 -> 存储 -> local_file_dir_obj . file.filename
    upload_file = files[0]   #  ??? 
    # print(f"[INFO] 上传文件：{files}, 类型为：{type(files[0])}")
    local_file_path_obj:Path =  local_file_dir_obj /upload_file.filename
    ...
    with local_file_path_obj.open("wb") as file_buffer:
        ...
        # 指定每次 10 MB = length = 1 * 1024 kb * 1024 mb * 10
    ...
    background_tasks.add_task(
        ...
        local_file_dir = str(local_file_dir_obj)
    )
...
def task_status(task_id:str):
    ...
    # done_list running_list 每次调用的时候,先存储!
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host=infra_config.settings_config.app_host, port=infra_config.settings_config.import_app_port)